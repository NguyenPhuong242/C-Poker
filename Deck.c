...
{
    assert(Deck_indexIsValid(deck, index1));
    assert(Deck_indexIsValid(deck, index2));
...
void Deck_sortBySuitFirst(Deck *deck)
{
    ...
    qsort(deck->cards, deck->length, sizeof deck->cards[0], reverseCompareCardBySuitFirst);
}

void Deck_sortByRankFirst(Deck *deck)
{

    qsort(deck->cards, deck->length, sizeof deck->cards[0], reverseCompareCardByRankFirst);
    // qsort(deck->cards, deck->length, sizeof(Card), reverseCompareCardByRankFirst);
}

// bool Deck_isSortedBySuitFirst (Deck const * deck) {
//    return deck == Deck_sortBySuitFirst(*deck);
// }

// bool Deck_isSortedByRankFirst (Deck const * deck) {
//     return deck == Deck_sortByRankFirst(*deck);
// }
